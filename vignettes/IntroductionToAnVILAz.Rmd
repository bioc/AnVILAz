...
We will also use a small log file for demonstration purposes. The `jupyter.log`
file is already present in our workspace directory.

```{r}
az_copy_to_storage("jupyter.log", "analyses/")
```
...
```{r}
avbackup("./test/", "analyses/test/", contentsOnly = TRUE)
```
...
```{r}
avrestore("analyses/test", contentsOnly = TRUE)
```
...
```{r}
avrestore("analyses/test", "test", contentsOnly = TRUE)
```
